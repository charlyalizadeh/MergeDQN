...
def select_action(state):
    ...
    if sample > 0.1:
        with torch.no_grad():
            ...
            values[0][env.action_done] = 0
            return policy_net(state).max(1).indices.view(1, 1)
    else:
        possible_actions = [i for i in range(env.nedge) if i not in env.action_done]
...
def plot_boxplot(show_result=False):
    plt.figure(1)
    if show_result:
        plt.title('Result')
    else:
    ...
    if len(all_episode_rewards) == 1:
        plt.boxplot([all_episode_rewards[-1], []])
    else:
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        #all_episode_rewards[-1].append(reward[0])

        if done:
            print(f"REWARD = {reward[0]}")
            print(f"NUMBER OF ACTIONS = {t}")
